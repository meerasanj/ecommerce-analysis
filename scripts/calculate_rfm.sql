...
-- This will be our reference point for calculating recency.
...
-- Make the 'max_purchase_date' from our CTE available to every row for the recency calculation.
...
# 1. Run the SQL query using DuckDB and convert the result to a Pandas DataFrame.
rfm_df = duckdb.query(query).to_df()

# 2. Save resulting DataFrame to new CSV file in the processed data folder.
